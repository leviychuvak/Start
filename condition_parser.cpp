...
shared_ptr<nodes::Node> ParseExpression(It& current, It end, unsigned precedence) {
  ...
  const map<LogicalOperator, unsigned> precedences = {
      {LogicalOperator::Or, 1}, {LogicalOperator::And, 2}
  };
  ...
  while (current != end && current->type != TokenType::PAREN_RIGHT) {
    ...
    const auto logical_operation = current->value == "AND" ? LogicalOperator::And
                                                           : LogicalOperator::Or;
